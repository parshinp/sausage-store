...
then sudo docker-compose stop green && sudo docker rm green && sudo docker-compose --env-file=.env.back up -d green
...
then sudo docker rm green && sudo docker-compose --env-file=.env.back up -d green
while [ "$( sudo docker container inspect -f '{{.State.Health.Status}}' green )" != "healthy" ]; do
        sleep 1;
done
exit;
...
then sudo docker-compose --env-file=.env.back up -d green
while [ "$( sudo docker container inspect -f '{{.State.Health.Status}}' green )" != "healthy" ]; do
        sleep 1;
done
sudo docker-compose stop blue; exit;
...
then sudo docker-compose --env-file=.env.back up -d blue

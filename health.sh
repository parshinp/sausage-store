...
then [ sudo docker pull gitlab.praktikum-services.ru:5050/parshin/sausage-store/sausage-backend:$VERSION ] && [ sudo docker-compose stop green ] && [ sudo docker-compose --env-file .env.back up -d green ] && [ sudo docker-compose stop blue ]
fi
